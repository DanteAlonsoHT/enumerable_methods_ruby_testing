...
symbol_test = %i[sym1 sym2]

path_test = %w[document memes doge]
...
describe Enumerable do
  describe '#my_each' do
    ...
    it 'returns when array was given' do
      expect(array_test.my_each { |i| i**2 }).to eql(array_test.each { |i| i**2 })
    end
  ...
  describe '#my_each_with_index' do
    ...
    it 'returns when array was given' do
      expect(array_test.my_each_with_index { |i| i**2 }).to eql(array_test.each { |i| i**2 })
    end
  ...
  describe '#my_none' do
    ...
    it "returns true when block doesn't return true for each one" do
      expect(path_test.my_none? { |name| !name.empty? }).to eql(path_test.none? { |name| !name.empty? })
    end
  end
  ...
  describe '#my_inject' do
    it 'returns a value when range was given' do
      expect(range_test.my_inject { |i, _obj| i**2 }).to eql(range_test.inject { |i, _obj| i**2 })
    end
  end
